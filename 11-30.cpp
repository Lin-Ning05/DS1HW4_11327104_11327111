//四還有一些小問題
//三應該差不多了(我只測到403)我有一點改版，你可能要稍微再看一下
...
class Kitchen {
  ...
  public:
    ...
    void SetAllorder(std::string &filename);
    void Work();
    bool ToFile(std::string outname);
...
void Kitchen::Work() {
    ...
    while (j <= all_size) {
        ...
        if (all && !(all -> dequeue(to_work))) {
            for (int i = 1 ; i <= number ; i++) {
                while (cook[i].GetQueue() -> dequeue(to_work)) {
                    cook[i].OrderOKK(to_work, abort_list, timeout_list);
                }
            }
            return;
        }
        j++;
        if (to_work.Arrival + to_work.Duration > to_work.Timeout) {//非法輸入，忽略
            total_order--;
            continue;
        }
